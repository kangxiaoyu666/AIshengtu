    ...
    <nav className={`fixed inset-x-0 top-0 z-50 h-16 transition-all duration-300 ${
      ...
      <div className="mx-auto flex h-full max-w-7xl items-center justify-between px-6">
        ...
        <Link href="/" className="flex items-center gap-2.5 group">
          <div className="w-9 h-9 rounded-xl bg-gradient-to-br from-blue-500 to-blue-600 flex items-center justify-center shadow-md shadow-blue-500/25 group-hover:shadow-lg group-hover:scale-105 transition-all">
            <Cpu className="h-5 w-5 text-white" />
          </div>
          <span className="text-lg font-extrabold gradient-text tracking-tight">椒图AI</span>
        </Link>
        ...
        <div className="flex items-center gap-2">
          <Link href="/wallet"
            className="hidden md:flex items-center gap-1.5 px-3 py-1.5 rounded-xl bg-amber-50 border border-amber-200 text-amber-600 hover:bg-amber-100 transition-all text-sm font-semibold shadow-sm">
          ...
          <Button onClick={() => router.push('/studio')}
            className="hidden md:inline-flex items-center gap-2 rounded-xl bg-gradient-to-r from-blue-500 to-blue-600 hover:from-blue-600 hover:to-blue-700 text-white border-0 h-9 px-5 shadow-md shadow-blue-500/25 btn-glow font-semibold text-sm">
      ...
      {mobileOpen && (
        <div className="md:hidden bg-white/95 backdrop-blur-xl border-t border-slate-200 px-4 py-4 space-y-2 shadow-lg">
          <Link href="/wallet" onClick={() => setMobileOpen(false)}
            className="flex items-center gap-3 px-4 py-3 rounded-xl bg-amber-50 border border-amber-200 text-amber-600 font-medium">
            <Coins className="h-4 w-4" />余额: {points.toLocaleString()} 点 <Plus className="h-3 w-3 text-amber-400 ml-auto" /></Link>
          {navLinks.map((link) => {
            const Icon = link.icon;
            return (
